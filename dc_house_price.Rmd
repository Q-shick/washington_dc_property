...
### __Linear Model: *All Variables*__
...
### __Handle Influential Points: *DFBETA*__

```{r}
# DFBETA
dfbeta.ROOMS <- dfbetaPlots(house.lm, terms = "ROOMS")
dfbeta.STORIES <- dfbetaPlots(house.lm, terms = "STORIES")
...
dfbetas.infl.obs <- names(dfbetas[abs(dfbetas[ , "ROOMS"]) 
                                  > dfbeta.thrhd, "ROOMS"])
dfbetas.infl.obs <- c(dfbetas.infl.obs, 
                      names(dfbetas[abs(dfbetas[ , "STORIES"]) 
                                  > dfbeta.thrhd, "STORIES"]))
...
### __Final Linear Model__

```{r}
# drop insignificant regressors
house.price <- select(house.price, -c("ROOMS", "EXTWALL"))
...
```{r}
temp<-lm(PRICE_TRANS ~ EYB+GBA+STRUCT+ROOF+RMDL+AC+WARD, data=house.price)
summary(temp)
```


```{r}
rsq.lm <- regsubsets(PRICE_TRANS ~ ., data = house.price, nbest = 3)
plot(rsq.lm, scale = "r2")
plot(rsq.lm, scale = "adjr2")
plot(rsq.lm, scale = "Cp")
```
...
```{r}
# cross validation
# cv.lm(df = house.price, house.final.lm, m = 3)
```
...
```{r}
# stepwise selection
step(null.lm, scope = list(lower = null.lm, upper = full.lm), direction = "both")
```


### __Advanced Method: *Ridge Regression*__

```{r}
# ridge regression
x <- house.price %>% select(-PRICE_TRANS) %>% data.matrix()
